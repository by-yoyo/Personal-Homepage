...
function RepoSortMenu({
	...
	const panelClassName = open
		? `${styles.sortPanel} ${styles.sortPanelOpen}`
		: styles.sortPanel;
	...
	return (
		<li className={styles.repoItem}>
			<div className={styles.repoTitleRow}>
				...
				<div
					className={styles.repoUpdatedInline}
					title={`${labels.updated} · ${updatedAt}`}
				>
					<span className={styles.repoUpdatedLabel}>{labels.updated}</span>
					<time
						className={styles.repoUpdatedValue}
			...
			<div className={styles.metaRow}>
				{repo.language ? (
					<span className={styles.chip}>{repo.language}</span>
				) : null}
				{lic ? <span className={styles.chipMuted}>{lic}</span> : null}
			</div>

			<dl className={styles.dateGrid}>
				<div className={styles.dateCell}>
					<dt>{labels.created}</dt>
					<dd>{formatRepoDate(repo.created_at, locale)}</dd>
				</div>
				<div className={styles.dateCell}>
					<dt>{labels.pushed}</dt>
	...
	return (
		<div className={styles.box}>
			<header className={styles.toolbar}>
				<h2 className={styles.cardTitle}>{labels.cardTitle}</h2>
				{hasRepos ? (
					<RepoSortMenu
						locale={locale}
						sortMode={sortMode}
